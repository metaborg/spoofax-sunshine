...
/**
 * 
 * Singleton service for automatic discovery and registration of
 * {@link ALanguage}. Given a location
 * on the filesystem this service searches for the packed ESV files and attempts
 * to load languages
 * based on the information given in those ESV files. Languages that are
 * discovered are registered
 * into the {@link LanguageService}. Although it is a singleton this service is
 * basically stateless.
 * 
 * 
...
public class LanguageDiscoveryService {
	private static final Logger logger = LogManager
			.getLogger(LanguageDiscoveryService.class.getName());

	private final static String[] ESV_EXTENS = new String[] { "packed.esv" };

	/**
	 * Look for languages inside the given location on the filesystem.
	 * Discovered languages are
	 * configured based on the information in the packed ESV files and the
	 ...
	 * 
	 * The file paths mentioned in the ESV file are treated as relative to the
	 ...
	 * @param location
	 * @param skipLoading
	 *            If <code>true</code> then language are returned but are not
	 *            loaded into the {@link LanguageService}.
	 * @return
	 */
	public Collection<ALanguage> discover(Path locationPath, boolean skipLoading) {
		assert locationPath != null;
		logger.debug("Auto-discovering languages at location {}",
				locationPath.toAbsolutePath());
		...
		Iterator<File> esvs = FileUtils
				.iterateFiles(location, ESV_EXTENS, true);
		while (esvs.hasNext()) {
			File esv = esvs.next();
			try {
				...
				ALanguage language = languageFromEsv(document, esv.toPath()
						.getParent().getParent());
		...
		if (!skipLoading)
			ServiceRegistry.INSTANCE().getService(LanguageService.class)
					.registerLanguage(languages);
		return languages;
	}

	/**
	 * Read the textual representation of the ESV ATerm present on the input
	 * stream.
	 * 
	 * @see org.strategoxt.imp.runtime.dynamicloading.DescriptorFactory#tryReadTerm
	 *      for original
	 *      implementation
	 * 
	 * @param input
	 ...
	 * @throws IOException
	 */
	private static IStrategoAppl tryReadESV(PushbackInputStream input)
			throws IOException {
	...
	/**
	 * Creates an {@link ALanguage} from the given
	 * {@link SunshineLanguageArguments}.
	 * 
	 * @param args
	 * @return
	 */
	public ALanguage languageFromArguments(SunshineLanguageArguments args) {
		...
		for (int i = 0; i < (numJars + numCtrees); i++) {
			if (i < numJars)
				compilerFiles[i] = FileSystems.getDefault().getPath(
						args.jars.get(i));
			else
				compilerFiles[i] = FileSystems.getDefault().getPath(
						args.ctrees.get(i - numJars));
		}
		SunshineMainArguments mainArgs = ServiceRegistry.INSTANCE().getService(
				LaunchConfiguration.class).mainArguments;
		ALanguage language = new Language(args.lang, extens, args.ssymb,
				FileSystems.getDefault().getPath(args.tbl), args.observer,
				compilerFiles, Files.createTempDir().toPath());
		language.registerBuilder(mainArgs.builder, mainArgs.builder,
				mainArgs.buildonsource, false);
		return language;
	}

	/**
	 * Creates a {@link ALanguage} from the given ATerm contents of the packed
	 * ESV file describing
	 * it.
	 * 
	 * @param document
	...
	public ALanguage languageFromEsv(IStrategoAppl document, Path basepath) {
		...
		ALanguage language = new Language(name, extens, startsymb, parsetbl,
				observer, codefiles.toArray(new Path[codefiles.size()]),
				basepath);

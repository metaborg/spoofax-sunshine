...
public class ResultApplAnalysisResult implements IStrategoParseOrAnalyzeResult {
	...
	final String filename = ((IStrategoString) resultTerm.getSubterm(0))
		.stringValue();
	...
	messages.addAll(MessageHelper.makeMessages(this.file,
		MessageSeverity.ERROR, errors));
	messages.addAll(MessageHelper.makeMessages(this.file,
		MessageSeverity.WARNING, warnings));
	messages.addAll(MessageHelper.makeMessages(this.file,
		MessageSeverity.NOTE, notes));
    }

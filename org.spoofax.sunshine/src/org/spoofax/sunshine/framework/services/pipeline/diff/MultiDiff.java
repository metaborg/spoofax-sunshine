 ...
 * 
 */
public class MultiDiff<T> extends AbstractCollection<Diff<T>> implements
	Iterable<Diff<T>> {
    ...
    @Override
    public Iterator<Diff<T>> iterator() {
	return new Iterator<Diff<T>>() {
	    ...
	    private Iterator<Diff<T>> modificationsIter = modifications
		    .iterator();

	    @Override
	    public boolean hasNext() {
		return additionsIter.hasNext() || deletionsIter.hasNext()
			|| modificationsIter.hasNext();
	    }
    ...
    @Override
    public boolean add(Diff<T> diff) {
	...
	default:
	    throw new UnsupportedOperationException(
		    "Unsupported DiffKind kind: " + diff.getDiffKind());
	}
    }

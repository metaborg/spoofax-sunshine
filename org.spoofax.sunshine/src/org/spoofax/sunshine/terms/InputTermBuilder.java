...
public class InputTermBuilder {
	...
	private IStrategoTerm mkSubListTarget(IStrategoTerm resultingAst, IStrategoList targetTerm, StrategoSubList node) {
		...
		if (firstChild == null || lastChild == null)
			return null;
		return new TermTreeFactory(Environment.INSTANCE().termFactory).createSublist(targetTerm, firstChild, lastChild);
	}
	...
	public IStrategoTuple makeInputTerm(IStrategoTerm node, boolean includeSubNode, IStrategoList termPath,
		...
		if (includeSubNode) {
			IStrategoTerm[] inputParts = { targetTerm, termPath, rootTerm, factory.makeString(path),
...
//	public IStrategoTerm makeInputTermRefactoring(IStrategoTerm userInput, IStrategoTerm node, boolean includeSubNode,
//			boolean source) {
...
//	}
	...
	/**
	 * Create an input term for a control rule, based on the IStrategoTerm syntax of the AST of the
	 * source file.
	 */
	public IStrategoTuple makeATermInputTerm(IStrategoTerm node, boolean includeSubNode, File file) {
		...
		if (includeSubNode) {
			...
			IStrategoTerm[] inputParts = { implodeATerm(node),
					StrategoTermPath.createPathFromParsedIStrategoTerm(node, runtime.getCompiledContext()),
					implodeATerm(getRoot(node)), factory.makeString(path), factory.makeString(absolutePath) };
			return factory.makeTuple(inputParts);
		} else {
...
//	/**
//	 * Gets the node furthest up the ancestor chain that has either the same character offsets or
//	 * has only one child with the same character offsets as the node given. Won't traverse up list
//	 * parents.
//	 * 
//	 * @param allowMultiChildParent
//	 *            Also fetch the first parent if it has multiple children (e.g., Call("foo",
//	 *            "bar")).
//	 */
...
//				&& (getParent(result).getSubtermCount() <= 1 || (getLeftToken(tryGetOrigin(getParent(result)))
//						.getStartOffset() >= startOffset && getRightToken(tryGetOrigin(getParent(result)))
...
//	/**
//	 * Gets a node that has either the same character offsets or has only one child with the same
//	 * character offsets as the node given, meeting the additional criteria that this node matches
...
//	 *            "bar")).
//	 */
//	public static final IStrategoTerm getMatchingNode(IStrategoTerm[] semanticNodes, IStrategoTerm node,
...
//			selectionNode = new TermTreeFactory(Environment.getTermFactory()).createSublist(
...
//		while (!isMatch && selectionNode != null && !selectionNode.isList() && getParent(selectionNode) != null

...
public class StrategoTermPath {
	...
	public static IStrategoList createPathFromParsedIStrategoTerm(final IStrategoTerm node, Context context) {
		...
		final IStrategoTerm marker = context.getFactory().makeString(COMPLETION_TOKEN);
...
//		Environment.logWarning("Could not find common ancestor for nodes: " + node1 + "," + node2);
	...
	private static IStrategoTerm tryCreateListCommonAncestor(IStrategoTerm commonAncestor, List<IStrategoTerm> ancestors1List, IStrategoTerm child2) {
		if (commonAncestor != child2 && commonAncestor.isList()) {
			...
			return new TermTreeFactory(Environment.INSTANCE().termFactory).createSublist((IStrategoList) commonAncestor, child1, child2); 
		} else {
			return commonAncestor;
		}
	}

...
/**
 * Maintains aterm paths, lists of nodes on the path to the root from a given
 * AST node.
 * 
...
public class StrategoTermPath {
	...
	for (int i = 0; i < pathInts.size(); i++) {
	    results.add(Environment.INSTANCE().termFactory.makeInt(pathInts
		    .get(i)));
	}
     ...
     * The resulting path relates to the actual AST, ignoring the 'appl' etc
     * constructors of the IStrategoTerm syntax.
     */
    public static IStrategoList createPathFromParsedIStrategoTerm(
	    final IStrategoTerm node, Context context) {
	...
	return (IStrategoList) position_of_term_1_0.instance.invoke(context,
		top, new Strategy() {
		    @Override
		    public IStrategoTerm invoke(Context context,
	...
	class TestOrigin extends Strategy {
		...
		if (hasImploderOrigin(current)) {
			...
			for (int i = 0, max = currentOrigin.getSubtermCount(); i < max; i++)
			 ...
			 // exists, but one of its subterms is origin-related to
			 // the originChild
		    if (current == origin1)
			return current;
		    if (nextBest == null && originChild != null) {
    ...
    /**
     * Find the common ancestor of two AST nodes, creating a SubListAstNode if
     * they are in the same list ancestor.
     */
	...
	for (IStrategoTerm n = node2, n2Child = node2; n != null; n2Child = n, n = getParent(n)) {
	    ...
	    if (node1Index != -1 && node1Ancestors.get(node1Index) == n) // common
									 // ancestor
									 // w/
									 // reference
									 // equality
	...
	// XXX
	// Environment.logWarning("Could not find common ancestor for nodes: " +
	// node1 + "," + node2);
	...
	if (commonAncestor != child2 && commonAncestor.isList()) {
	    ...
	    return new TermTreeFactory(Environment.INSTANCE().termFactory)
		    .createSublist((IStrategoList) commonAncestor, child1,
			    child2);
	} else {
	    return commonAncestor;
	}
	...
	for (int c = 0, size = selectionPath.size(); c < size; c++) {
		...
		// XXX
		// Environment.logException("Unable to recover old selection AST in "
		// + oldParent, new ArrayIndexOutOfBoundsException(i));
		...
		return findIdenticalSubtree(oldAst, newAst, selection);
	    } else if (oldParent.getSubtermCount() > newParent
		    .getSubtermCount()) {
		i = max(0,
			i
				- (oldParent.getSubtermCount() - newParent
					.getSubtermCount()));
	    ...
	    if (c == size - 1) {
		return findCorrespondingSubtreeResult(oldAst, newAst,
			oldSubtree, newSubtree, selection, newParent, i);
	    }
	...
	IStrategoTerm start = findCorrespondingSubtree(newAst,
		selection.getFirstChild());
	IStrategoTerm end = findCorrespondingSubtree(newAst,
		selection.getLastChild());
	return findCommonAncestor(start, end);
    }

    private static boolean constructorEqual(IStrategoTerm first,
	    IStrategoTerm second) {
	IStrategoConstructor firstCons = tryGetConstructor(first);
	return firstCons == null ? tryGetConstructor(second) == null
	...
	if (!newSubtree.equals(oldSubtree)) {
	    ...
	    IStrategoTerm exactMatch = findIdenticalSubtree(oldAst, newAst,
		    selection);
	    if (exactMatch != null)
		return exactMatch;
	    ...
	    if (constructorEqual(newSubtree, oldSubtree)
		    && (containsMultipleCopies(newAst, newSubtree) || findSubtree(
			    oldAst, newSubtree, true) == null)) {
    ...
    /**
     * Finds a unique, identical 'selection' subtree in 'newAst.' Returns null
     * in case of multiple candidate subtrees.
     */
	...
	visitor.visit(ast);
	return !allowMultipleResults && visitor.isMultiple ? null
		: visitor.result;
    }

    private static boolean containsMultipleCopies(IStrategoTerm ast,
	    IStrategoTerm subtree) {

...
public class BuilderInputTermFactoryLink implements
    ...
    @Override public void sink(MultiDiff<AnalysisFileResult<IStrategoTerm, IStrategoTerm>> product) {
        ...
        for(Diff<AnalysisFileResult<IStrategoTerm, IStrategoTerm>> diff : product) {
            if(diff.getPayload().file().getName().getPath().equals(path.getName().getPath())) {
                select = diff;
                break;
            } else {
                logger
                    .trace("Input file {} does not match prebaked file {}, skipping.", diff.getPayload().file(), path);
            }
        }
        if(select != null) {
            ...
            if(!errors_exist || ignoreErrors) {
                ...
                IStrategoTerm ast =
                    onSource && select.getPayload().parseResult() != null ? select.getPayload().parseResult().result
                ...
                BuilderInputTerm payload =
                    new BuilderInputTerm(launch.termFactory, ast, select.getPayload().file(), launch.projectDir);

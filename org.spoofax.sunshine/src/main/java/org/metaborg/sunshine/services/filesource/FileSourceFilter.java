...
public class FileSourceFilter extends ALinkManyToMany<FileObject, FileObject> {
    ...
    /**
     * Constructs a {@link FileSourceFilter} that will only pass through instances of {@link File} whose absolute path
     * returned by {@link File#getAbsolutePath()} matches the given regular expression. If the given regex is
     * <code>null</code> then all files are passed through.
     * 
     * @param regex
     */
    ...
    @Override public MultiDiff<FileObject> sinkWork(MultiDiff<FileObject> input) {
        if(regex == null) {

...
public class DirMonitorTest {
	...
	public void testGetChanges6() throws IOException, InterruptedException {
		...
		os.close();
		// deletion & addition
		assertTrue(new File(dir, "lol.foo").renameTo(new File(dir, "movedhere.foo")));
	...
	public void testGetChanges8() throws IOException, InterruptedException {
		...
		assertEquals(NUMFILES / 2, mon.getChanges().getDiff(DiffKind.ADDITION).size());
		int idx = 1;
		for (File file : files) {
			if (idx > NUMFILES / 2)
				break;
		...
		assertEquals(NUMFILES / 2, mon.getChanges().getDiff(DiffKind.DELETION).size());
	}
	...
	public void testPersist1() throws IOException {
		...
		assertEquals(NUMFILES, mon.getChanges().getDiff(DiffKind.ADDITION).size());
		mon = null;
		int idx = 1;
		for (File file : files) {
			if (idx > NUMFILES / 2)

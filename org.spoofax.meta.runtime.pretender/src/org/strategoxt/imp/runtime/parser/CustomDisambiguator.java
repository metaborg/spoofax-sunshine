...
import static org.spoofax.sunshine.parser.impl.SourceAttachment.getFile;
...
public class CustomDisambiguator {
	...
	private final OLDSGLRParseController controller;

	private final String[] functions;

	private WeakReference<StrategoObserver> runtime;

	public CustomDisambiguator(OLDSGLRParseController controller, String[] functions) {
	...
	public IStrategoTerm disambiguate(IStrategoTerm ast) {
		...
		if (Environment.isMainThread()) {
			...
			controller.scheduleParserUpdate(OLDSGLRParseController.REPARSE_DELAY, false);
	...
	private StrategoObserver getRuntime() {
		StrategoObserver result = runtime == null ? null : runtime.getParseTable();
		try {
			if (result == null) {

...
public class SunshineFileSystemManagerProvider extends DefaultFileSystemManagerProvider {
    private final SunshineMainArguments mainArguments;

    @Inject public SunshineFileSystemManagerProvider(SunshineMainArguments mainArguments) {
        super();
    ...
    @Override protected void setBaseFile(DefaultFileSystemManager manager) throws FileSystemException {
        try {
            // Absolute path
            final FileObject direct = manager.resolveFile(mainArguments.project);
            manager.setBaseFile(direct);
        } catch(FileSystemException e) {
            // Relative path
            final FileObject current = manager.resolveFile(System.getProperty("user.dir"));

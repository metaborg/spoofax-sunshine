...
public class record_time_native_0_1 extends Strategy {
    ...
    @Override public IStrategoTerm invoke(Context context, IStrategoTerm time, IStrategoTerm name) {
        final String nameStr = ((IStrategoString) name).stringValue();
        final Long timeLong = Math.round(((IStrategoReal) time).realValue() * 1000);
